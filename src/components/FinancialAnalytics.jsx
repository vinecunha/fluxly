...
function TabSelector({ tab, onTabChange }) {
  const tabs = [
    { id: 'gasto', label: 'Gastos', Icon: TrendingDown, color: 'rose' },
    { id: 'renda', label: 'Renda', Icon: TrendingUp, color: 'emerald' },
    { id: 'investimento', label: 'Reservas', Icon: PiggyBank, color: 'blue' },
  ]
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-1 flex gap-1">
      {tabs.map(({ id, label, Icon, color }) => {
        ...
        const colorClasses = {
          rose: isActive ? 'bg-rose-50 text-rose-600' : 'text-gray-500 hover:text-rose-500',
          emerald: isActive ? 'bg-emerald-50 text-emerald-600' : 'text-gray-500 hover:text-emerald-500',
          blue: isActive ? 'bg-blue-50 text-blue-600' : 'text-gray-500 hover:text-blue-500',
        }
        return (
          <button
            key={id}
...
// ─── Card de Total (Destaque principal) ──────────────────────────────────────
function TotalCard({ title, value, trend, trendLabel, avgValue, color, icon: Icon }) {
  const isPositive = value >= 0
  const colorMap = {
    emerald: { text: 'text-emerald-600', bg: 'bg-emerald-50', iconBg: 'bg-emerald-100' },
    rose: { text: 'text-rose-600', bg: 'bg-rose-50', iconBg: 'bg-rose-100' },
      ...
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-2">
            <div className={`w-8 h-8 rounded-xl flex items-center justify-center ${colors.iconBg}`}>
          ...
          {trend && (
            <div className="flex items-center gap-1.5 mt-2">
              <span className={`text-[9px] font-black ${trend > 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                {trend > 0 ? '↑' : '↓'} {Math.abs(trend).toFixed(0)}%
              </span>
              <span className="text-[8px] text-gray-400">{trendLabel}</span>
            </div>
          )}
        </div>
        {avgValue > 0 && (
          <div className="text-right">
            <p className="text-[7px] font-black text-gray-300 uppercase">Média 3m</p>
            <p className="text-[10px] font-black text-gray-500">{fmtK(avgValue)}</p>
          </div>
        )}
      </div>
    </div>
...
// ─── Seção de Distribuição (Pizza + Lista) ───────────────────────────────────
function DistributionSection({ grouped, viewTotal, tab, selectedCategory, onCategorySelect }) {
...
function EvolutionChart({ allTransactions, tab, currentDate }) {
  const meses = useMemo(() => {
    const result = []
    for (let i = 5; i >= 0; i--) {
  ...
  const cor = tab === 'renda' ? '#10b981' : tab === 'investimento' ? '#3b82f6' : '#ef4444'
  const corLight = tab === 'renda' ? '#d1fae5' : tab === 'investimento' ? '#dbeafe' : '#fee2e2'
  ...
  const titulo = tab === 'renda' ? 'Evolução da Renda' : tab === 'investimento' ? 'Evolução das Reservas' : 'Evolução dos Gastos'
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Últimos 6 meses</p>
        </div>
        {Math.abs(delta) >= 1 && (
      ...
      <p className="text-sm font-black text-gray-800 mb-4">{titulo}</p>
...
function CategoryRanking({ allTransactions, tab, currentDate }) {
  const dados = useMemo(() => {
    const build = (mes, ano) => {
      const txs = (allTransactions || []).filter(t => {
        const td = new Date(t.data + 'T12:00:00')
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
      <div className="flex items-center gap-2 mb-3">
        ...
        <p className="text-[9px] font-black text-gray-400 uppercase tracking-widest">Top Categorias</p>
      </div>
      <div className="flex items-center justify-between mb-4">
...
export function FinancialAnalytics({ transactions = [], allTransactions = [], currentDate }) {
  ...
  const filtered = useMemo(() => (transactions || []).filter(t => {
    ...
    if (!ok) return false
    if (selectedDate) {
      const ref = (t.data_pagamento ? new Date(t.data_pagamento).toLocaleDateString('en-CA') : t.data) || ''
  ...
  const handleDaySelect = (dateStr) => {
    ...
    if (dateStr) setSelectedCategory(null)
  }

  const handleCategorySelect = (cat) => {
    setSelectedCategory(prev => prev === cat ? null : cat)
    if (cat) setSelectedDate(null)
  }
  ...
  const tabConfig = {
    ...
    investimento: { title: 'Reservas', value: viewTotal, color: 'blue', icon: PiggyBank, trendLabel: 'vs média 3 meses' },
  }
  ...
  return (
    <section className="space-y-4 animate-in fade-in duration-300">
        ...
        <div className="flex items-center justify-between px-4 py-2.5 bg-slate-50 rounded-2xl border border-slate-100">
          <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-black text-slate-700">
              {selectedDate
                ? `Filtrando por: ${new Date(selectedDate + 'T12:00:00').toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' })}`
      ...
      <TotalCard
        ...
        trend={trendDiff}
        trendLabel={currentConfig.trendLabel}
        avgValue={avgTotal}
