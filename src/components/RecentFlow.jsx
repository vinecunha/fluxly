...
import { ChevronLeft, ChevronRight, Search, X, ArrowUpDown, Filter, CheckCircle2 } from 'lucide-react'
...
export function RecentFlow({ transactions = [], onEdit, onDelete }) {
  ...
  // Categorias disponíveis nos dados atuais
  ...
  const availableTipos = [
    { id: 'renda', label: 'Renda', color: 'emerald' },
    { id: 'gasto_diario', label: 'Gasto', color: 'rose' },
    { id: 'fixa', label: 'Fixa', color: 'blue' },
    { id: 'esporadica', label: 'Extra', color: 'orange' },
    { id: 'reserva', label: 'Reserva', color: 'purple' },
  ]
  ...
  const filtered = useMemo(() => {
    ...
    if (filterCat) {
      list = list.filter(t => t.categoria === filterCat)
    }

    if (filterTipo) {
      list = list.filter(t => t.tipo === filterTipo)
    }
  ...
  const handleFilterCat = (v) => { setFilterCat(prev => prev === v ? null : v); setCurrentPage(1) }
  const handleFilterTipo = (v) => { setFilterTipo(prev => prev === v ? null : v); setCurrentPage(1) }
      ...
      <div className="p-5 pb-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="relative">
              <select
                ...
                className="appearance-none bg-gray-50 border-none text-[10px] font-black text-gray-500 pr-5 pl-2 py-1.5 rounded-xl outline-none cursor-pointer"
              >
                {SORT_OPTIONS.map(o => (
            ...
            <button
              ...
              className={`flex items-center gap-1 text-[10px] font-black uppercase px-2 py-1.5 rounded-xl transition-all ${
        ...
        <div className="relative">
          ...
          <input
            ...
            className="w-full pl-9 pr-8 py-2.5 bg-gray-50 rounded-xl text-xs font-medium outline-none focus:ring-2 focus:ring-indigo-300"
          />
          {search && (
                ...
                <div className="flex flex-wrap gap-1.5">
                  {availableCategories.map(cat => (
                    <button
                      ...
                      className={`flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-black uppercase transition-all ${
                        filterCat === cat
                          ? 'bg-indigo-600 text-white'
                          : 'bg-gray-100 text-gray-500'
                      }`}
                    >
                      <span className={categoryIcons[cat]?.color}>{categoryIcons[cat]?.icon || '📦'}</span>
                      {cat}
                    </button>
                  ))}
                </div>
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between px-5 py-3 border-t border-gray-50">
          <button
            ...
            className="p-1.5 rounded-lg bg-gray-50 disabled:opacity-30 active:scale-90 transition-all"
          >
...
function SwipeableFlowItem({ transaction: t, todayStr, onEdit, onDelete }) {
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      <div
        className={`relative flex items-center gap-3 px-5 py-3 bg-white transition-transform ${swiping ? '' : 'duration-200'}`}
        style={{ transform: `translateX(${offset}px)` }}
        onTouchStart={handleTouchStart}
        onTouchMove={handleTouchMove}
        onTouchEnd={handleTouchEnd}
      >
        <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${categoryIcons[t.categoria]?.color || 'bg-gray-100 text-gray-600'}`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-0.5">
            ...
            {isToday && (
              <span className="text-[8px] font-black text-amber-600 bg-amber-50 px-1.5 py-0.5 rounded-md uppercase">Hoje</span>
            )}
            {t.categoria && (
        ...
        <div className="flex-shrink-0 text-right">
          <p className={`text-sm font-black ${colorClass}`}>
            {prefix} R$ {Number(t.valor).toFixed(2)}
          </p>
          {t.pago !== undefined && (
            <span className={`text-[9px] font-black uppercase ${t.pago ? 'text-emerald-500' : 'text-amber-500'}`}>
              {t.pago ? <CheckCircle2 size={13} className="text-emerald-500 inline" /> : <span className="text-[9px] font-black uppercase text-amber-500">Pendente</span>}
            </span>
          )}
        </div>
      </div>
