...
export function RecentFlow({ transactions = [], onEdit, onDelete }) {
  ...
  const TOTAL_CARDS = [
    ...
    { key: 'fatura',   label: 'Faturas',   value: totals.fatura,   Icon: CreditCard,   active: 'bg-slate-700 text-white',   idle: 'bg-slate-50',   text: 'text-slate-700',   icon: 'text-slate-500'  },
  ].filter(c => c.value > 0)
  ...
  return (
    <div className="relative overflow-hidden group">
      <div className="absolute inset-0 flex items-center justify-between px-5 pointer-events-none">
        <span className="text-[10px] font-black py-5 px-2 bg-slate-800 text-white uppercase">Editar ✏️</span>
        <span className="text-[10px] font-black py-5 px-2 bg-rose-700 text-white uppercase">🗑️ Excluir</span>
      </div>

      <div
        className={`relative flex items-center gap-3 px-4 py-3 bg-white ${swiping ? '' : 'transition-transform duration-200'}`}
