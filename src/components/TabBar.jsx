...
export function BottomNav({ activeTab, onChangeTab, onAddNew, transactions = [] }) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white/90 backdrop-blur-md border-t border-gray-100 shadow-[0_-4px_24px_rgba(0,0,0,0.06)]">
      <div className="flex items-center justify-around w-full max-w-2xl mx-auto px-2">
        {/* Botão Início */}
        ...
        <div className="flex flex-col items-center pb-2">
          ...
          >
            ...
            <div className="absolute -top-10 left-1/2 -translate-x-1/2 bg-gray-900 text-white text-[9px] font-black px-2.5 py-1.5 rounded-xl whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 shadow-lg z-50">
              {smart.label}
      ...
      {/* Se quiser adicionar Metas como sexto botão, descomente abaixo e ajuste o grid */}
      {/* <div className="flex justify-around items-center w-full max-w-2xl mx-auto px-2 border-t border-gray-50 pt-1">
        <NavButton label="Metas" Icon={Target} active={activeTab === TABS.METAS} onClick={() => onChangeTab(TABS.METAS)} />
      </div> */}
    </nav>
  )
}
...
function NavButton({ label, Icon, active, onClick }) {
  return (
    <button
      onClick={onClick}
      className={`flex flex-col items-center py-2 px-3 transition-all active:scale-90 ${
